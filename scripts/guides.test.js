...
// 05.09.2026, прогоны #116/#117 (трижды подряд, всегда «line 113»): модель
// вставляет в перевод НЕэкранированную кавычку — «Expected ',' or '}' after
// property value». Кавычка внутри строки отличима от закрывающей тем, что за
// закрывающей идёт только , } ] или : — всё остальное значит «текст продолжается».
t('неэкранированная кавычка внутри строки экранируется, а не рвёт строку', () => {
  assert.deepStrictEqual(repairJson('{"title":"Trump calls it a "big win" for crypto","x":1}'),
    { title: 'Trump calls it a "big win" for crypto', x: 1 });
});

t('неэкранированная кавычка перед переводом строки внутри абзаца', () => {
  assert.deepStrictEqual(repairJson('{"p":["он сказал "да"\nи ушёл","b"]}'),
    { p: ['он сказал "да"\nи ушёл', 'b'] });
});

t('настоящая закрывающая кавычка перед запятой/скобкой/двоеточием не трогается', () => {
  assert.deepStrictEqual(repairJson('{"a":"x" , "b":["y"] ,"c":{"d":"z"}}'),
    { a: 'x', b: ['y'], c: { d: 'z' } });
});
