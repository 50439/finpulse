...
// --- Квота считается по КАЛЕНДАРНЫМ суткам UTC, а не скользящим окном -----
// 06.09: утренний прогон в 10:07 UTC пропустил выпуск — «за окно 20 ч уже
// 2 статьи». Вчерашние вышли в 14:11 и 19:41 UTC (поздно, из-за задержек
// GitHub), и скользящее окно перенесло их на сегодня. Каждый день публикация
// уезжала бы всё позже — тот же дрейф, только при двух статьях в сутки.
// Календарные сутки обнуляют счётчик в 00:00 UTC, а «две подряд» держит
// отдельный newsMinGapHours.
console.log('\n--- квота по календарным суткам UTC ---');
{
  const now = Date.parse('2026-09-06T10:07:00Z');
  const cfg2 = { newsPerDay: 2 };
  const yesterday = [{ date: '2026-09-05T14:11:00Z' }, { date: '2026-09-05T19:41:00Z' }];
  const cases = [
    [yesterday, false, 'две вчерашние статьи не блокируют утро', cfg2],
    [yesterday.concat([{ date: '2026-09-06T08:00:00Z' }]), false, 'одна сегодняшняя из двух — можно вторую', cfg2],
    [yesterday.concat([{ date: '2026-09-06T08:00:00Z' }, { date: '2026-09-06T09:00:00Z' }]), true,
      'две сегодняшние — стоп', cfg2],
    [[{ date: '2026-09-06T00:05:00Z' }], true, 'ночная статья считается за сегодня', { newsPerDay: 1 }],
    [[{ date: '2026-09-05T23:55:00Z' }], false, 'вчерашняя ночная не считается за сегодня', { newsPerDay: 1 }]
  ];
  for (const [arts, want, name, cfg] of cases) {
    const got = quotaFull(arts, cfg, now);
    const ok = got === want;
    if (!ok) fail++;
    console.log((ok ? '  OK  ' : '  FAIL') + ' ' + name + '  ->  ' + got);
  }
